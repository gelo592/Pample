$(function() {
  ...
  .done(function(data) {
    ...
    pample.gameBoard = {
      ...
      mapDimensions: {
        fullWidth: data.tileMap.length,
        fullHeight: data.tileMap[0].length,
        visibleWidth: Math.floor(canvas.width / this.tileWidth),
        visibleHeight: Math.floor(canvas.height / (this.tileHeight / 2)),
        movementAreaLeft: Math.floor((canvas.width / this.tileWidth) / 4),
        movementAreaUp: Math.ceil((canvas.height / this.tileHeight) / 4),
        movementAreaRight: 3 * Math.floor((canvas.width / this.tileWidth) / 4),
        movementAreaDown: 3 * Math.ceil((canvas.height / this.tileHeight) / 4),
        xOffset: 0,
        yOffset: 0
      },
        ...
        switch(keycode) {
          case 37:
            if(player.x <= 0) {
              ...
              if(this.player.x < this.mapDimensions.movementAreaLeft && moMap.left) { //
